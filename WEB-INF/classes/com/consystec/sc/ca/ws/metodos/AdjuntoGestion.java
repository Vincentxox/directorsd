...
import com.consystec.sc.ca.ws.cliente.AdjuntoGestionCliente;
...
import com.consystec.sc.sv.ws.metodos.CtrlAdjuntoGestion;

public class AdjuntoGestion extends ControladorBase {
    String TOKEN = "";

    private static final Logger log = Logger.getLogger(AdjuntoGestion.class);
     ...
     */
    public OutputAdjuntoGestion cargarAdjunto(InputAdjuntoGestion objDatos) {
        OutputAdjuntoGestion objRespuestaAdjuntoGestion = new OutputAdjuntoGestion();
        Connection conn = null;
        Respuesta objRespuesta = new Respuesta();
        String metodo = "cargarAdjunto";
        String url = "";
            ...
            try {
                ...
                if ("LOGIN".equals(TOKEN)) {
                    ...
                    objRespuesta = getMensaje(Conf_Mensajes.MSJ_SESION_EXPIRO_12, null, this.getClass().toString(),
                            metodo, null, Conf_Mensajes.ORIGEN_SERVICIOS_OP);
                    objRespuestaAdjuntoGestion.setRespuesta(objRespuesta);
                	...
                	if(isFullStack(objDatos.getCodArea())){
                		...
                		CtrlAdjuntoGestion recurso = new CtrlAdjuntoGestion();
                		objRespuestaAdjuntoGestion=recurso.cargarAdjunto(objDatos);
               
                	}else{
	                    log.trace("obteniendo url");
	                    url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_CARGA_ADJUNTO);
	                    log.trace("url:" + url);
	                    if (url == null || "null".equals(url) || "".equals(url)) {
	                        ...
	                        AdjuntoGestionCliente wsAdjuntoGestion = new AdjuntoGestionCliente();
	                        wsAdjuntoGestion.setServerUrl(url);
	                        log.trace("consume servicio");
	                        objRespuestaAdjuntoGestion = wsAdjuntoGestion.cargarAdjuntoGestion(objDatos);
	                    }
                	}
                }
            } catch (SQLException e) {
        ...
        objRespuestaAdjuntoGestion.setToken(TOKEN);
        return objRespuestaAdjuntoGestion;
    }

    public OutputAdjuntoGestion getAdjunto(InputAdjuntoGestion objDatos) {
        OutputAdjuntoGestion objRespuestaAdjuntoGestion = new OutputAdjuntoGestion();
        Connection conn = null;
        Respuesta objRespuesta = new Respuesta();
        String metodo = "getAdjunto";
        String url = "";
        log.trace("inicia a validar valores...");
        objRespuesta = validarDatos(objDatos);
        if (objRespuesta == null) {
            ...
            try {
                ...
                if ("LOGIN".equals(TOKEN)) {
                	...
                	if(isFullStack(objDatos.getCodArea())){
                		CtrlAdjuntoGestion recurso = new CtrlAdjuntoGestion();
                		  log.trace("consume metodo");
		                    objRespuestaAdjuntoGestion =recurso.getAdjunto(objDatos);
                	}else{
		                // obteniendo url de servicio
		                log.trace("obteniendo url");
		                url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_GET_ADJUNTO);
		                log.trace("url:" + url);
		                if (url == null || "null".equals(url) || "".equals(url)) {
		                    objRespuesta = getMensaje(Conf_Mensajes.URL_NOTFOUND_9, null, this.getClass().toString(),
		                            metodo, null, Conf_Mensajes.ORIGEN_SERVICIOS_OP);
		                    ...
		                    wsAdjuntoGestion.setServerUrl(url);
		                    log.trace("consume servicio");
		                    objRespuestaAdjuntoGestion = wsAdjuntoGestion.getAdjuntoGestion(objDatos);
		                }
                	}
                }
            } catch (SQLException e) {
    ...
    public OutputAdjuntoGestion delAdjunto(InputAdjuntoGestion objDatos) {
        OutputAdjuntoGestion objRespuestaAdjuntoGestion = new OutputAdjuntoGestion();
        Connection conn = null;
        Respuesta objRespuesta = new Respuesta();
        String metodo = "delAdjunto";
        String url = "";
        log.trace("inicia a validar valores...");
        objRespuesta = validarDatos(objDatos);
        if (objRespuesta == null) {
                ...
                if ("LOGIN".equals(TOKEN)) {
                    ...
                    objRespuestaAdjuntoGestion.setRespuesta(objRespuesta);
                   	...
                   	if(isFullStack(objDatos.getCodArea())){
                		CtrlAdjuntoGestion recurso = new CtrlAdjuntoGestion();
                		  log.trace("consume metodo");
		                    objRespuestaAdjuntoGestion =recurso.delAdjunto(objDatos);
                	}else{
	                    // obteniendo url de servicio
	                    log.trace("obteniendo url");
	                    url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_DEL_ADJUNTO);
	                    log.trace("url:" + url);
	                    if (url == null || "null".equals(url) || "".equals(url)) {
	                        objRespuesta = getMensaje(Conf_Mensajes.URL_NOTFOUND_9, null, this.getClass().toString(),
	                                metodo, null, Conf_Mensajes.ORIGEN_SERVICIOS_OP);
	                        ...
	                        wsAdjuntoGestion.setServerUrl(url);
	                        log.trace("consume servicio");
	                        objRespuestaAdjuntoGestion = wsAdjuntoGestion.delAdjuntoGestion(objDatos);
	                    }
                	}
                }
            } catch (SQLException e) {
