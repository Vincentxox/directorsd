...
import com.consystec.sc.ca.ws.cliente.TrasladoCliente;
...
public class Traslado extends ControladorBase {

    private static final Logger log = Logger.getLogger(Traslado.class);

    /***
     * Validando que no vengan par\u00E9metros nulos
     */
    public Respuesta validarDatos(InputTraslado objDatos) {
     ...
     */
    public OutputTraslado creaTraslado(InputTraslado objDatos) {
        OutputTraslado objRespuestaTraslado = new OutputTraslado();
        Connection conn = null;
        Respuesta objRespuesta = new Respuesta();
        String metodo = "getTraslado";
        String url = "";
        ...
        if (objRespuesta == null) {
            ...
            try {
                ...
                { 
            		...
            		CtrlTraslado recurso=new CtrlTraslado();
            		objRespuestaTraslado=recurso.getDatos(objDatos, Conf.METODO_PUT);
                }
                else
                {
                	 // obteniendo url de servicio
                    log.trace("obteniendo url");
                    url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_CREATRASLADO);
//                    url="http://localhost:8181/WS_SIDRA_PA/sidra/rest/creaTraslado/";//TODO cambiar url quemada
                    log.trace("url:" + url);
                    ...
                    if (url == null || url.equals("null") || "".equals(url)) {
                        objRespuesta = getMensaje(Conf_Mensajes.URL_NOTFOUND_9, null, this.getClass().toString(), metodo,
                                null, Conf_Mensajes.ORIGEN_SERVICIOS_OP);
                        objRespuestaTraslado.setRespuesta(objRespuesta);
                    } else {
                        log.trace("url valida");
                        TrasladoCliente wsTraslado = new TrasladoCliente();
                        wsTraslado.setServerUrl(url);
                        log.trace("consume servicio");
                        objRespuestaTraslado = wsTraslado.creaTraslado(objDatos);
                    }
                }
