...
import com.consystec.sc.ca.ws.cliente.FolioDTSCliente;
...
public class FolioDTS extends ControladorBase {

    private static final Logger log = Logger.getLogger(FolioDTS.class);

    /***
     * Validando que no vengan par\u00E9metros nulos
     */
    public Respuesta validarDatos(InputConfiguracionFolio objDatos) {
    ...
    /**
     * M\u00E9todo para crear folios de bodegas DTS
     */
    public OutputConfiguracionFolio crearFolioBodDTS(InputConfiguracionFolio objDatos) {
        OutputConfiguracionFolio objRespuestaFolio = new OutputConfiguracionFolio();
        Connection conn = null;
        ...
        if (objRespuesta == null) {
            try {
                ...
                url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_ADDFOLIOBODDTS);
                log.trace("url:" + url);
                if ( url == null || "".equals(url)) {
                    ...
                    FolioDTSCliente wsFolioBodegaV = new FolioDTSCliente();
                    wsFolioBodegaV.setServerUrl(url);
                    objRespuestaFolio = wsFolioBodegaV.crearFolioBodDTS(objDatos);
                }
            } catch (SQLException e) {
                log.error(e,e);
                objRespuesta = getMensaje(e.getErrorCode(), e.getMessage(), this.getClass().toString(), metodo, null,
    ...
    /**
     * M\u00E9todo para dar de baja folios de bodegas DTS
     */
    public OutputConfiguracionFolio bajaFolioBodDTS(InputConfiguracionFolio objDatos) {
        OutputConfiguracionFolio objRespuestaFolio = new OutputConfiguracionFolio();
        Connection conn = null;
        Respuesta objRespuesta = new Respuesta();
        String metodo = "getBodegas";
        ...
        if (objRespuesta == null) {
            try {
                conn = getConnLocal();
                // obteniendo url de servicio
                url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_BAJAFOLIOBODDTS);
                log.trace("url:" + url);
                if (url == null || "".equals(url)) {
                    objRespuesta = getMensaje(Conf_Mensajes.URL_NOTFOUND_9, null, this.getClass().toString(), metodo,
                            null, Conf_Mensajes.ORIGEN_SERVICIOS_OP);
                    ...
                    FolioDTSCliente wsFolioBodegaV = new FolioDTSCliente();
                    wsFolioBodegaV.setServerUrl(url);
                    objRespuestaFolio = wsFolioBodegaV.bajaFolioBodDTS(objDatos);
                }
            } catch (SQLException e) {
                log.error(e,e);
                objRespuesta = getMensaje(e.getErrorCode(), e.getMessage(), this.getClass().toString(), metodo, null,
     ...
     * M\u00E9todo para dar de baja folios de bodegas DTS
     */
    public OutputConfiguracionFolio getFolioBodDTS(InputConfiguracionFolio objDatos) {
        OutputConfiguracionFolio objRespuestaFolio = new OutputConfiguracionFolio();
        Connection conn = null;
        Respuesta objRespuesta = new Respuesta();
        String metodo = "getBodegas";
        ...
        if (objRespuesta == null) {
            try {
                conn = getConnLocal();
                // obteniendo url de servicio
                url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_GETFOLIOBODDTS);
                log.trace("url:" + url);
                if (url == null ||"".equals(url)) {
                    objRespuesta = getMensaje(Conf_Mensajes.URL_NOTFOUND_9, null, this.getClass().toString(), metodo,
                            null, Conf_Mensajes.ORIGEN_SERVICIOS_OP);
                    ...
                    FolioDTSCliente wsFolioBodegaV = new FolioDTSCliente();
                    wsFolioBodegaV.setServerUrl(url);
                    objRespuestaFolio = wsFolioBodegaV.getFolioBodDTS(objDatos);
                }
            } catch (SQLException e) {
                log.error(e,e);
                objRespuesta = getMensaje(e.getErrorCode(), e.getMessage(), this.getClass().toString(), metodo, null,
