...
import com.consystec.sc.ca.ws.cliente.ArticuloPrecioCliente;
...
import com.consystec.sc.sv.ws.metodos.CtrlArticuloPrecio;


public class ArticuloPrecio extends ControladorBase{
	private static final Logger log = Logger.getLogger(ArticuloPrecio.class);
	
	public Respuesta validarDatos(InputArticuloPrecio objDatos) {
	...
	public OutputArticuloPrecio crearArticuloPrecio(InputArticuloPrecio objDatos) {
        OutputArticuloPrecio objRespuestaBuzon = new OutputArticuloPrecio();
        Connection conn = null;
                ...
                if (isFullStack(objDatos.getCodArea()))
                 {
                	...
                	CtrlArticuloPrecio recurso=new CtrlArticuloPrecio();
                	objRespuestaBuzon=recurso.crearArticuloPrecio(objDatos);
                 }
                else
                {
                	 // obteniendo url de servicio
                    url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_CREA_ARTICULO_PRECIO);
                    ...
                    if (url == null || "null".equals(url) || "".equals(url)) {
                        ...
                        ArticuloPrecioCliente wsCliente = new ArticuloPrecioCliente();
                        wsCliente.setServerUrl(url);
                        objRespuestaBuzon = wsCliente.crearArticuloPrecio(objDatos);
                    }
                }
	...
	public OutputArticuloPrecio modificaArticuloPrecio(InputArticuloPrecio objDatos) {
        OutputArticuloPrecio objRespuestaPrecio = new OutputArticuloPrecio();
        Connection conn = null;
        Respuesta objRespuesta = new Respuesta();
        String metodo = "crearArticuloPrecio";
        String url = "";
        ...
        if (objRespuesta == null) {
            try {
                ...
                if (isFullStack(objDatos.getCodArea())){
                	log.trace("consume metodo");
                	CtrlArticuloPrecio recurso=new CtrlArticuloPrecio();
                	objRespuestaPrecio= recurso.putArticuloPrecio(objDatos);
                }else{
            	   // obteniendo url de servicio
                   url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_MOD_ARTICULO_PRECIO);

                   log.trace("url:" + url);
                   if (url == null || "null".equals(url) || "".equals(url)) {
                       objRespuesta = getMensaje(Conf_Mensajes.URL_NOTFOUND_9, null, this.getClass().toString(), metodo,
                               null, Conf_Mensajes.ORIGEN_SERVICIOS_OP);
                       objRespuestaPrecio.setRespuesta(objRespuesta);
                   } else {
                       ArticuloPrecioCliente wsCliente = new ArticuloPrecioCliente();
                       wsCliente.setServerUrl(url);
                       objRespuestaPrecio = wsCliente.modArticuloPrecio(objDatos);
                   }  
               }
        ...
        return objRespuestaPrecio;
    }
	
	 public OutputArticuloPrecio getArticuloPrecios(InputArticuloPrecio objDatos) {
	        OutputArticuloPrecio objRespuestaPrecio = new OutputArticuloPrecio();
	        Connection conn = null;
	        Respuesta objRespuesta = new Respuesta();
	        ...
	        if (objRespuesta == null) {
	            try {
	                ...
	                if (isFullStack(objDatos.getCodArea())){
	                	log.trace("consume metodo");
	                	CtrlArticuloPrecio recurso=new CtrlArticuloPrecio();
	                	objRespuestaPrecio=recurso.getArticuloPrecio(objDatos);
	                }else{
	            	   // obteniendo url de servicio
	                   url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_GET_ARTICULO_PRECIO);
	                   ...
	                   if (url == null || "null".equals(url) || "".equals(url)) {
	                       ...
	                       ArticuloPrecioCliente wsCliente = new ArticuloPrecioCliente();
	                       wsCliente.setServerUrl(url);
	                       objRespuestaPrecio = wsCliente.getArticuloPrecio(objDatos);
	                   }   
	               }
