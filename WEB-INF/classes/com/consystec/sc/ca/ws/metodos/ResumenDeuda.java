...
import com.consystec.sc.ca.ws.cliente.ResumenDeudaCliente;
...
import com.consystec.sc.sv.ws.metodos.CtrlGetDeuda;
import com.consystec.sc.sv.ws.metodos.CtrlTransaccionesDeuda;

public class ResumenDeuda extends ControladorBase {
     ...
     */
    public Respuesta validarDatos(InputSolicitud objDatos) {
        ...
        if ( "".equals(objDatos.getToken()) || "".equals((objDatos.getToken().trim()))) {
            objRespuesta = new Respuesta();
     ...
     */
    public OutputDeuda getDeuda(InputSolicitud objDatos) {
        OutputDeuda objRespuestaDeuda = new OutputDeuda();
        Connection conn = null;
        ...
        if (objRespuesta == null) {
            try {
                ...
                if(isFullStack(objDatos.getCodArea())){
                	 CtrlGetDeuda recurso = new CtrlGetDeuda();
                	 objRespuestaDeuda=recurso.getResumenDeuda(objDatos);
                }else{
	                // obteniendo url de servicio
	                url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_GETRESUMENDEUDA);
	                ...
	                if (url == null || url.equals("null") || "".equals(url)) {
	                    ...
	                    objRespuesta = getMensaje(Conf_Mensajes.URL_NOTFOUND_9, null, this.getClass().toString(), metodo,
	                            null, Conf_Mensajes.ORIGEN_SERVICIOS_OP);
	                    objRespuestaDeuda.setRespuesta(objRespuesta);
	                } else {
	                    ResumenDeudaCliente wsCliente = new ResumenDeudaCliente();
	                    wsCliente.setServerUrl(url);
	                    objRespuestaDeuda = wsCliente.getResumenDeuda(objDatos);
	                }
                }
            } catch (SQLException e) {
                log.error(e, e);
        ...
        return objRespuestaDeuda;
    }

    public Respuesta validarDatos(InputTransDeuda objDatos) {
        Connection conn = null;
        Respuesta objRespuesta = null;
        String metodo = "validarDatos";
     ...
     * M\u00E9todo para obtener informaci\u00F3n de buzones
     */
    public OutputTransDeuda getTransaccionesDeuda(InputTransDeuda objDatos) {
        OutputTransDeuda objRespuestaDeuda = new OutputTransDeuda();
        Connection conn = null;
        Respuesta objRespuesta = new Respuesta();
        String metodo = "getTransaccionesDeuda";
        String url = "";
        log.trace("inicia a validar valores...");
        objRespuesta = validarDatos(objDatos);
        if (objRespuesta == null) {
            try {
                ...
                if (isFullStack(objDatos.getCodArea())){
                	  CtrlTransaccionesDeuda recurso = new CtrlTransaccionesDeuda();
                	  objRespuestaDeuda =recurso.getDatos(objDatos);
                }else{
	                // obteniendo url de servicio
	                url = Util.getURLWSLOCAL(conn, objDatos.getCodArea(), Conf.SERVICIO_LOCAL_GET_TRANSACCIONES_DEUDA);
	                ...
	                if (url == null || "null".equals(url) || "".equals(url)) {
	                    ...
	                    ResumenDeudaCliente wsCliente = new ResumenDeudaCliente();
	                    wsCliente.setServerUrl(url);
	                    objRespuestaDeuda = wsCliente.getTransaccionesDeuda(objDatos);
	                }
                }
            } catch (SQLException e) {
                log.error(e, e);
